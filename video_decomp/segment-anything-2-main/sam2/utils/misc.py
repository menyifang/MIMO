...
def _load_frame_as_tensor(img_pil, image_size):
    img_np = np.array(img_pil.convert("RGB").resize((image_size, image_size)))
    if img_np.dtype == np.uint8:  # np.uint8 is expected for JPEG images
        img_np = img_np / 255.0
    else:
...
def load_video_fixed_fps(vid_path, target_fps=30):
    ...
    # fps = reader.get_meta_data()['fps']  # Get the original video's fps
...
def load_video_frames_fixed_fps(
    frames,
    image_size,
    offload_video_to_cpu,
    img_mean=(0.485, 0.456, 0.406),
    ...
    You can load a frame asynchronously by setting `async_loading_frames` to `True`.
    """
    # frames = load_video_fixed_fps(video_path)
    ...
    images = torch.zeros(num_frames, 3, image_size, image_size, dtype=torch.float32)
    for n, frame in enumerate(tqdm(frames, desc="frame converting")):
