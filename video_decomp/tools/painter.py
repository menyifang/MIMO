...
def show_automask_anno(anns, input_box, text_box_list=None):
    ...
    if input_box is not None:
        ...
        line_thickness = 3
        line_type = 4
        box_left_top = input_box[:2]
        box_right_bottom = input_box[2:]
        cv2.rectangle(img, tuple(box_left_top), tuple(box_right_bottom), line_color, line_thickness, line_type)

    if text_box_list is not None:
        for text_box in text_box_list:
            box_left_top = text_box[:2]
            box_right_bottom = text_box[2:]
            line_color = (255, 0, 0)
            cv2.rectangle(img, tuple(box_left_top), tuple(box_right_bottom), line_color, line_thickness, line_type)
