...
if __name__ == '__main__':
    ...
    for dir in dir_list:
        ...
        paths = [
            # '/mnt/workspace/myf272609/qingyao/mycode/MIMO_infer/assets/video_template/movie_BruceLee/bk.mp4',
            # '/mnt/workspace/myf272609/qingyao/mycode/MIMO_infer/assets/video_template/shorts_kungfu_match/bk.mp4',
            # '/mnt/workspace/myf272609/qingyao/mycode/MIMO_infer/assets/video_template/shorts_kungfu_desert/bk.mp4',
            '/mnt/workspace/myf272609/qingyao/mycode/MIMO_infer/assets/video_template/dance_indoor_1/bk.mp4',
        ...
        for path in paths:
            ...
            for i in range(len(frames)):
                ...
                # x1, x2, y1, y2 = 10,90,10,160 # movie_BruceLee
                # x1, x2, y1, y2 = 10,110,10,255 

                # if i>=left_top_start and i<left_top_end:
                #     x1, x2, y1, y2 = 10,110,10,268 
                #     mask[x1:x2, y1:y2,  0] = 255
                #     mask[x1:x2, y1:y2, 1] = 255
                #     mask[x1:x2, y1:y2, 2] = 255

                # if i>=left_top_end:
                #     x1, x2, y1, y2 = -115,-10,-268,-10 
                #     mask[x1:x2, y1:y2, 0] = 255
                #     mask[x1:x2, y1:y2, 1] = 255
                #     mask[x1:x2, y1:y2, 2] = 255

                # x1, x2, y1, y2 = 440,-1100,260,-250 
                x1, x2, y1, y2 = 350,-1050,50,-40 
                mask[x1:x2, y1:y2, 0] = 255
                mask[x1:x2, y1:y2, 1] = 255
                mask[x1:x2, y1:y2, 2] = 255

                # frames_mask.append(mask)
                frames_mask.append(Image.fromarray(mask))


            # frames_res = process(args, device, frames, frames_mask, fps, size, video_name)
            while True:
                try:
                    frames_res = process(args, device, frames, frames_mask, fps, size, video_name)
                    torch.cuda.empty_cache()
            ...
            print('size after inpaint:', frames_res[0].shape)
            frames_res1 = []
            for img_idx, frame_res in enumerate(frames_res):

                frame_res = np.array(frame_res)
                frame_res = cv2.resize(frame_res, size, interpolation=cv2.INTER_AREA)
                
                frames_res1.append(frame_res)

            imageio.mimsave('res.mp4', frames_res1, fps=30, quality=8, macro_block_size=1)
